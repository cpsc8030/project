...
renderView1.ViewSize = [1920, 1680]
...
transformFilter = Transform(Input=step_full_046500vtk)
transformFilter.Transform.Translate = [200.0, 30.0, 0.0]
SetActiveSource(transformFilter)

# create a new 'Glyph'
glyph1 = Glyph(registrationName='Glyph1', Input=transformFilter,
    GlyphType='Arrow')
...
renderView1.GetActiveCamera().Azimuth(-15)

# cam = renderView1.GetActiveCamera()
# cam.SetPosition(cam.GetPosition()[0] - 100, cam.GetPosition()[1], cam.GetPosition()[2])
# cam.SetFocalPoint(cam.GetFocalPoint()[0] - 100, cam.GetFocalPoint()[1], cam.GetFocalPoint()[2])
...
transformFilter = Transform(Input=step_full_046500vtk)
transformFilter.Transform.Translate = [-20.0, 0.0, 0.0]
SetActiveSource(transformFilter)
...
renderView1.CameraViewUp = [0.20528001779217242, 0.2221186909017186, 0.9531649392667316]
renderView1.CameraParallelScale = 172.3211396844726
renderView1.GetActiveCamera().Azimuth(-165)
...
WriteImage(dir_out + fn.replace('.vtk', '.png'), size=(1920, 1680))
