...
dir_output = '/Users/dmitry/Dev/workspace/clemson/sv/project/SciVisContest23/viz-no-network/timeseries/sample/images/'
...
step = (threshold['end'] - threshold['start']) / len(files)
...
for i, fn in enumerate(files):
    ...
    # create a new 'Legacy VTK Reader'
    step_full_046500vtk = LegacyVTKReader(registrationName=fn, FileNames=[dir_files + fn])
    ...
    connected_dendritesLUT.RescaleTransferFunction(0.0, 4.0)

    # Rescale transfer function
    connected_dendritesPWF.RescaleTransferFunction(0.0, 4.0)

    # Rescale 2D transfer function
    connected_dendritesTF2D.RescaleTransferFunction(0.0, 4.0, 0.0, 4.0)
    ...
    renderView1.GetActiveCamera().Azimuth(-angle_delta * i)

    Render()
    print(fn.replace('.vtk', '.png'))
    WriteImage(dir_output + fn.replace('.vtk', '.png'))
